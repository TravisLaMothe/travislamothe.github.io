...
(function() {
  ...
  function init() {
    // TODO
    qsa('input[name="animal"]').forEach((animal) => {
      animal.addEventListener("change", makeRequest);
    });
  }
  ...
  function makeRequest() {
    ...
    let baseUrl = "https://courses.cs.washington.edu/courses/cse154/webservices/pets/ajaxpets.php";
    
    removeAnimals();

    fetch(baseUrl + "?animal=" + this.value)
      .then(checkStatus)
      .then(res => res.text())
      .then(showAnimals)
      .catch(console.error);
  ...
  function removeAnimals() {
    qsa("#pictures img").forEach((image) => {
